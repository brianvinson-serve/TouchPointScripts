...
**Status:** RESOLVED 2026-08-31. `RECIPIENT_PEOPLE_IDS` updated and `PREVIEW_MODE` flipped to `False` 2026-08-30; the `MorningBatch` block was in fact pasted live (contrary to this note's earlier assumption) -- but the first scheduled Monday run (2026-08-31) still didn't send. **Root cause: script name mismatch.** `MorningBatch` calls `model.CallScript("CM_AttendanceDashboardEmail")`, but the live Special Content Python script was saved as `CM_AttendanceReportEmail`. Since `model.CallScript` is fire-and-forget with no return value (see the M2 finding below), calling a name that doesn't exist is a silent no-op -- no error, no `EmailQueue` row, nothing visible anywhere. Confirmed via `dbo.EmailQueue`: SM's 8/31 Monday send had a matching row (queued/started/sent all ~07:42); CM had zero rows in that table, ever, at any date. **Fixed 2026-08-31:** Brian renamed the live script to `CM_AttendanceDashboardEmail` (matching `MorningBatch`, this repo's file name, and the `SM_AttendanceDashboardEmail` convention), flipped `PREVIEW_MODE` back to `False`, and ran it live -- confirmed sent (real recipient inbox + a new `dbo.EmailQueue` row with `Sent` populated). Next Monday's automated run should now work end-to-end; worth a quick `EmailQueue` spot-check on the next scheduled run to be sure.
