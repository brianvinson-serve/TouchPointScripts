...
**Confirmed live 2026-09-07** via `RPC_AttendanceRoster`'s involvement picker, which renders a sub-group count per involvement: the counts populate against real RPC data and Brian confirmed the resulting rosters read correctly. Division `31` ("AD Classes/Meetings/Groups", Program `1119`) holds **156 active involvements**, a useful scale marker for any division-driven picker. Note the caveat below is visible in that same data — e.g. `AD: Adult Ministries Volunteer Training 2026` (237 members) reports **21 sub-groups**, which is far more consistent with volunteer-scheduling slots than with true sub-groups. Treat a high sub-group count on a volunteer/training involvement as probable scheduling data, not table assignments.

**Surfaced in reporting as of 2026-09-07:** `attendance-roster-report/RPC_AttendanceRoster.py` reads both tables to offer an involvement's sub-groups as a roster column, sort order, and page-break grouping (`OrgMemMemTags` joined to `MemberTags` on `MemberTagId`, filtered by `OrgId`; the composite PK means one row per member-per-sub-group with no fan-out). **Not yet live-verified** — that these rows are what the involvement UI shows as "SubGroups" for a real RPC involvement is assumed from the structure below, not confirmed against a known involvement. Note the caveat in the next paragraph: RPC's existing `MemberTags` usage skews toward event-RSVP options and volunteer-scheduling slots, which share this table, so a live pass should check whether those appear as spurious "sub-groups."
...
- **`People.City` and `People.State` do NOT exist** — confirmed live 2026-08-30 (`Invalid column name 'City'`/`'State'`, SQL error 207) when `attendance-roster-report/RPC_AttendanceRoster.py` tried an address column built from those. Address/city/state/zip almost certainly live on `Families` instead (per `People.FamilyId -> Families.FamilyId` above), not yet confirmed. Before assuming any specific address column name, run: `SELECT TABLE_NAME, COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME IN ('People', 'Families') AND (COLUMN_NAME LIKE '%Address%' OR COLUMN_NAME LIKE '%City%' OR COLUMN_NAME LIKE '%State%' OR COLUMN_NAME LIKE '%Zip%') ORDER BY TABLE_NAME, COLUMN_NAME`.
...
| SQL Scripts | RPC_FamilyEngagementDashboardValidation | Built 2026-09-03, run live same day (see "Family Engagement / Giving" above for findings). Pre-deploy validation for RPC_FamilyEngagementDashboard -- confirmed the `EngagementScoreTag.StatusFlagId` -> `StatusFlagList.Flag` join (8/9 flags) and surfaced the TotalScore-double-counting bug that was then fixed. Re-run needed on the corrected 3-signal version before RPC_FamilyEngagementDashboard is deployed. See `family-engagement-dashboard/README.md`. |
| Python Scripts | RPC_FamilyEngagementDashboard | Built 2026-09-03, corrected same day (TotalScore removed as a voting signal), live-tested and confirmed working 2026-09-03 -- see "Family Engagement / Giving" above. Two-sided family engagement worklist (Sliding Out / Newly Engaging), 3-signal (Volunteer flags / Kids flag / Giving, >=2-of-3) -- see `family-engagement-dashboard/README.md`. Hard-gated to `AUTHORIZED_PEOPLE_IDS = (47110, 1673)` (Brian Vinson, Alan Michael) via `model.UserPeopleId` -- anyone else gets a generic no-access message, no query run. |
